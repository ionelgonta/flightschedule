...
function DestinationsMatrixView({ destinations }: DestinationsMatrixViewProps) {
  ...
  const getRoutesForDestinationAndDay = (destination: string, day: DayOfWeek): WeeklyScheduleData[] => {
    ...
    return destData.routes.filter(route => route.weeklyPattern[day])
  }
                    ...
                    <td colSpan={9} className="px-4 py-4">
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-7 gap-4">
                          {days.map((day) => {
                            const dayRoutes = getRoutesForDestinationAndDay(dest.destination, day)
                            return (
                                    ...
                                    {dayRoutes.map((route, idx) => (
                                      <div key={idx} className="bg-white rounded p-2 text-xs">
                                        ...
                                        <div className="text-gray-500 truncate">
                                          {route.airline.length > 15 ? `${route.airline.substring(0, 15)}...` : route.airline}
                                        </div>
                                      </div>
                                    ))}
                                  </div>
                          ...
                          {dayRoutes.map((route, idx) => (
                            <div key={idx} className="flex items-center justify-between text-xs">
                              ...
                              <div className="text-gray-500 text-right">
                                {route.airline.length > 20 ? `${route.airline.substring(0, 20)}...` : route.airline}
                              </div>
                            </div>
                          ))}
                        </div>
