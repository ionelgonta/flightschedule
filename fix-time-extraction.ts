 ...
 * Cauta pattern-uri comune pentru ora de plecare in boarding pass-uri
 */
function extractDepartureTimeFromPDFText(pdfText: string): string | null {
  ...
  while ((match = timeRegex.exec(singleLineText)) !== null) {
    ...
    if (hours >= 0 && hours <= 23 && minutes >= 0 && minutes <= 59) {
      ...
      // Exclude daca e parte dintr-o data
  ...
  if (foundTimes.length > 0) {
    ...
    console.log('[TIME] All times: ' + foundTimes.map(t => t.time + '(' + t.source + ')').join(', '));
    const best = foundTimes[0];
    console.log('[TIME] Selected: ' + best.time + ' (' + best.source + ')');
    return best.time;
  }

  console.log('[TIME] No departure time found');
  return null;
}
