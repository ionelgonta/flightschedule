...
export default function DeparturesPage({ params }: DeparturesPageProps) {
  ...
  const fetchFlights = async (filters?: FlightFilters) => {
    try {
      setLoading(true)
      setError(null)
      const response = await airportsService.getDepartures(airport.code, filters)
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-green-600 dark:text-green-400">
                    {flights.filter(f => f.status === 'scheduled' || f.status === 'active').length}
                  </span>
                </div>
                <div className="flex justify-between">
