...
async function testApiKey(apiKey: string): Promise<{ valid: boolean; error?: string }> {
  try {
    ...
    const testUrl = `https://api.market/aerodatabox/v1/flights/airports/icao/LROP/arrivals/${today}T00:00/${today}T23:59`;
    ...
    if (response.ok) {
      ...
      return { valid: false, error: 'API key nu a fost găsit sau nu are permisiuni' };
    } else {
