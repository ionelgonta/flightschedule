 ...
 */
export function detectBarcodeType(rawData: string): BarcodeType {
 ...
 */
export async function generateUniversalBarcode(
  rawData: string,
  type?: BarcodeType,
  ...
  try {
    ...
    const defaultOptions: BarcodeGeneratorOptions = {
      scale: 3,
      includetext: false,
      textxalign: 'center'
    };
 ...
 */
export async function generateBoardingPassBarcode(
  bcbpData: string,
  forceType?: BarcodeType
): Promise<BarcodeResult> {
 ...
 */
export function validateBarcodeData(rawData: string): { valid: boolean; reason?: string } {
  if (!rawData || rawData.trim().length === 0) {
