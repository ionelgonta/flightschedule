...
class CacheManager {
  ...
  async initialize(): Promise<void> {
    ...
    // CRITICAL FIX: Clean up any corrupted nested flight data structures
    // This prevents the need to delete persistent cache by fixing corruption at source
    const cleanedEntries = await this.cleanupCorruptedFlightData()
    if (cleanedEntries > 0) {
      console.log(`[Cache Manager] Auto-cleaned ${cleanedEntries} corrupted cache entries during initialization`)
    }
    
    // Initialize persistent flight system first with timeout protection
    if (persistentFlightSystem) {
      try {
        console.log('[Cache Manager] Initializing persistent flight system...')
        await Promise.race([
          persistentFlightSystem.initialize(),
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Persistent system initialization timeout')), 10000)
          )
        ])
        console.log('[Cache Manager] Persistent flight system initialized successfully')
      } catch (error) {
        console.error('[Cache Manager] Failed to initialize persistent flight system:', error)
        console.log('[Cache Manager] Continuing without persistent system...')
        // Continue without persistent system if it fails
      }
    }

    // Initialize historical cache manager with timeout protection
    if (historicalCacheManager) {
      try {
        console.log('[Cache Manager] Initializing historical cache manager...')
        await Promise.race([
          historicalCacheManager.initialize(),
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Historical cache initialization timeout')), 10000)
          )
        ])
        console.log('[Cache Manager] Historical cache manager initialized successfully')
      } catch (error) {
        console.error('[Cache Manager] Failed to initialize historical cache manager:', error)
        console.log('[Cache Manager] Continuing without historical cache...')
  ...
  private async loadConfig(): Promise<void> {
    ...
    try {
      ...
      // Ensure all required properties exist, merge with defaults from environment variables
      const defaultFlightInterval = Math.floor((parseInt(process.env.NEXT_PUBLIC_AUTO_REFRESH_INTERVAL || '600000') / 1000) / 60) // Convert ms to minutes
      
      this.config = {
        flightData: {
          cronInterval: defaultFlightInterval // Always use environment variable
        },
        analytics: {
      ...
      // Configurație default - folosește environment variables
      const defaultFlightInterval = Math.floor((parseInt(process.env.NEXT_PUBLIC_AUTO_REFRESH_INTERVAL || '600000') / 1000) / 60) // Convert ms to minutes
  ...
  private async startCronJobs(): Promise<void> {
    if (!this.config) {
      console.warn('[Cache Manager] No configuration available, using default configuration')
    // Use default configuration as fallback
    this.config = {
      flightData: { cronInterval: 60 }, // 60 minutes
      analytics: { cronInterval: 1440, cacheMaxAge: 86400 }, // 24 hours, 1 day cache
      aircraft: { cronInterval: 10080, cacheMaxAge: 604800 }, // 7 days, 1 week cache
      weather: { cronInterval: 30 } // 30 minutes
    }
    }

    // Validate configuration has all required properties, use defaults if missing
    const defaultConfig = {
      flightData: { cronInterval: 60 },
      analytics: { cronInterval: 1440, cacheMaxAge: 86400 },
      aircraft: { cronInterval: 10080, cacheMaxAge: 604800 },
      weather: { cronInterval: 30 }
    }

    // Ensure config is properly initialized
    this.config.flightData = this.config.flightData || defaultConfig.flightData
    this.config.analytics = this.config.analytics || defaultConfig.analytics
    this.config.aircraft = this.config.aircraft || defaultConfig.aircraft
    this.config.weather = this.config.weather || defaultConfig.weather

    if (!this.config.flightData.cronInterval) {
      console.warn('[Cache Manager] Missing flightData.cronInterval, using default:', defaultConfig.flightData.cronInterval)
      this.config.flightData = defaultConfig.flightData
    }
    if (!this.config.analytics.cronInterval) {
      console.warn('[Cache Manager] Missing analytics.cronInterval, using default:', defaultConfig.analytics.cronInterval)
      this.config.analytics = defaultConfig.analytics
    }
    if (!this.config.aircraft.cronInterval) {
      console.warn('[Cache Manager] Missing aircraft.cronInterval, using default:', defaultConfig.aircraft.cronInterval)
      this.config.aircraft = defaultConfig.aircraft
    }
    if (!this.config.weather.cronInterval) {
      console.warn('[Cache Manager] Missing weather.cronInterval, using default:', defaultConfig.weather.cronInterval)
      this.config.weather = defaultConfig.weather
    }

    // Oprește job-urile existente pentru a preveni duplicate
    this.stopCronJobs()
    
    console.log('[Cache Manager] Starting cron jobs with intervals:', {
      flightData: this.config.flightData.cronInterval + ' minutes',
      analytics: this.config.analytics.cronInterval + ' minutes',
      aircraft: this.config.aircraft.cronInterval + ' minutes',
   ...
   * FIXED: Prevents nested structure corruption by checking existing format
   */
  private async integrateWeatherIntoFlightCache(weatherData: { [cityName: string]: any }): Promise<void> {
    ...
    for (const [entryId, entry] of this.cacheData.entries()) {
      if (entry.category === 'flightData') {
        ...
        if (cityName && weatherData[cityName]) {
          // CRITICAL FIX: Determine current data format and handle appropriately
          let actualFlightData: any[] = []
          let currentWeatherInfo: any = null
          let needsUpdate = false
          
          // Case 1: Data is already in new format { flights: [...], weather_info: {...} }
          if (entry.data && typeof entry.data === 'object' && 'flights' in entry.data && 'weather_info' in entry.data) {
            ...
            currentWeatherInfo = entry.data.weather_info
            
            // Fix any nested corruption in flights array
            while (actualFlightData && typeof actualFlightData === 'object' && 'flights' in actualFlightData && !Array.isArray(actualFlightData)) {
              actualFlightData = (actualFlightData as any).flights
              console.log('[Cache Manager] Fixed nested flights corruption in entry:', entryId)
            }
            
            // Only update if weather data is newer or different
            const newWeatherTime = new Date(weatherData[cityName].lastUpdated).getTime()
            const currentWeatherTime = currentWeatherInfo?.lastUpdated ? new Date(currentWeatherInfo.lastUpdated).getTime() : 0
            needsUpdate = newWeatherTime > currentWeatherTime
            
          }
          // Case 2: Data is in old format { flights: [...] } (without weather_info)
            ...
            needsUpdate = true // Always add weather info if missing
          }
          // Case 3: Data is in legacy format (direct array)
          else if (Array.isArray(entry.data)) {
            actualFlightData = entry.data
            needsUpdate = true // Convert to new format with weather
          }
          // Case 4: Invalid or corrupted data
          else {
            console.warn(`[Cache Manager] Invalid data format in entry ${entryId}, skipping weather integration`)
            continue
          }
          
          // Ensure we have a valid flights array
          if (!Array.isArray(actualFlightData)) {
            console.warn(`[Cache Manager] Could not extract valid flights array from entry ${entryId}, skipping`)
            continue
          }
          
          // Only update if needed (new weather data or missing weather info)
          if (needsUpdate) {
    ...
    if (updatedEntries > 0) {
      ...
      console.log('[Cache Manager] No flight cache entries needed weather updates')
    }
  }
  ...
  private async fetchAndCacheFlightData(
    ...
    try {
      ...
      if (response.success && response.data && response.data.length > 0) {
        ...
        // API failed or returned no data - implement retry logic
        console.log(`[Cache Manager] API failed or returned no data for ${airportCode} ${type}`)
        
        if (oldEntries.length === 0) {
          // No existing data - create a temporary error entry that will be retried
          console.log(`[Cache Manager] No existing data for ${airportCode} ${type}, will retry on next cron run`)
          // Don't save error entries to cache - let next cron run retry
        } else {
          // Keep existing data and log retention
          console.log(`[Cache Manager] Keeping existing cache data for ${airportCode} ${type} (${oldEntries.length} entries)`)
          
          // Check if existing data is very old (more than 24 hours)
          const oldestEntry = oldEntries.reduce((oldest, entry) => 
            new Date(entry.createdAt) < new Date(oldest.createdAt) ? entry : oldest
          )
          const ageHours = (Date.now() - new Date(oldestEntry.createdAt).getTime()) / (1000 * 60 * 60)
          
          if (ageHours > 24) {
            console.warn(`[Cache Manager] Existing data for ${airportCode} ${type} is ${Math.round(ageHours)} hours old - API recovery needed`)
          }
        }
      }
  ...
  /**
   * Obține toate aeroporturile suportate din România și Moldova (coduri IATA)
   * DOAR aeroporturile românești și moldovenești pentru cache/statistici
   */
  private async getAllSupportedAirports(): Promise<string[]> {
    try {
      ...
      // Folosește DOAR codurile IATA pentru România și Moldova
      ...
      console.log(`Found ${iataCodes.length} supported airports for cache/statistics: ${iataCodes.join(', ')}`)
      return iataCodes
    } catch (error) {
      console.error('Error getting supported airports:', error)
      // Fallback la lista hardcodată în caz de eroare - DOAR România și Moldova
      return ['OTP', 'TSR', 'CLJ', 'IAS', 'BCM', 'BBU', 'SBZ', 'SUJ', 'CND', 'OMR', 'SCV', 'TGM', 'BAY', 'CRA', 'ARW', 'GHV', 'RMO']
    }
  }
  ...
  /**
   * Clean up corrupted nested flight data structures
   * This fixes existing cache entries that have been corrupted by previous weather integration
   * CRITICAL: This prevents the need to delete persistent cache by fixing corruption at source
   */
  async cleanupCorruptedFlightData(): Promise<number> {
    console.log('[Cache Manager] Cleaning up corrupted flight data structures...')
    ...
    for (const [entryId, entry] of this.cacheData.entries()) {
      if (entry.category === 'flightData') {
        ...
        let cleanedData = entry.data
        
        // Check if data has nested flights structure
        if (entry.data && typeof entry.data === 'object' && 'flights' in entry.data) {
          let flightData = entry.data.flights
          
          // Detect and fix nested flights corruption
          ...
          if (needsCleanup) {
            ...
            const weatherInfo = entry.data.weather_info || null
            
            cleanedData = weatherInfo ? {
              flights: Array.isArray(flightData) ? flightData : [],
              weather_info: weatherInfo
            } : (Array.isArray(flightData) ? flightData : [])
            
            console.log(`[Cache Manager] Fixed corrupted nested structure in entry: ${entryId}`)
          }
        }
        ...
        if (needsCleanup) {
          ...
          this.cacheData.set(entryId, cleanedEntry)
          cleanedEntries++
        }
      }
    }
    ...
    if (cleanedEntries > 0) {
      await this.saveCacheData()
      console.log(`[Cache Manager] Cleaned up ${cleanedEntries} corrupted flight cache entries`)
    } else {
      console.log('[Cache Manager] No corrupted flight cache entries found')
    }
...
// This backup cache manager was causing additional API requests
// Use fixedCacheManager instead

/*
// Auto-initialize cache manager on server startup
if (typeof window === 'undefined') {
  // Use setTimeout to avoid blocking the import
  setTimeout(async () => {
    try {
      console.log('[Cache Manager] Starting auto-initialization...')
      await cacheManager.initialize()
      console.log('[Cache Manager] Auto-initialization completed successfully')
    } catch (error) {
      console.error('[Cache Manager] Auto-initialization failed:', error)
      // Retry after 30 seconds
      setTimeout(async () => {
        try {
          console.log('[Cache Manager] Retrying initialization...')
          await cacheManager.initialize()
          console.log('[Cache Manager] Retry initialization completed')
        } catch (retryError) {
          console.error('[Cache Manager] Retry initialization failed:', retryError)
        }
      }, 30000)
    }
  }, 100)
