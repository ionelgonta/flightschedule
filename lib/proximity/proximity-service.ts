  ...
  async findNearbyAirports(searchCity: string): Promise<ProximityResult> {
    ...
    if (!geocodeResult) {
      return {
        ...
        message: `Nu am putut găsi locația pentru "${searchCity}". Verifică ortografia.`
      }
    }
    ...
    if (nearbyAirports.length === 0) {
      ...
      return {
        ...
        nearbyAirports: [],
        hasDirectFlights: false,
        message: `Nu există zboruri directe în această zonă. Cel mai apropiat aeroport cu zboruri directe este ${closest.city} (${formatDistance(closestDist)}).`
      }
    }
    ...
    return {
      ...
      hasDirectFlights: false,
      message: `Am găsit ${nearbyAirports.length} ${nearbyAirports.length === 1 ? 'aeroport' : 'aeroporturi'} cu zboruri directe în apropiere de ${searchCity}.`
    }
  }
